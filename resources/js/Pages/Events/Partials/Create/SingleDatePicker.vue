...
const onEndTimeChange = () => {
    if (!selectedDate.value || isNaN(new Date(selectedDate.value).getTime())) {
        console.warn('Invalid selectedDate:', selectedDate.value);
        return;
    }

    if (enableEndTime.value) {
        const hour = parseInt(selectedEndHour.value || '0', 10);
        const minute = parseInt(selectedEndMinute.value || '0', 10);

        let endDateTime = setHours(setMinutes(new Date(selectedDate.value), minute), hour);
